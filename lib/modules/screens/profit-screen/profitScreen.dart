...
//                       for (var item in items) {
//                         double purchaseRate =
//                             double.tryParse(item['purchaseRate'].toString()) ??
//                                 0.0;
//                         double saleRate =
                ...
                builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      double totalProfit = 0.0;
                      double cashDiscount = double.tryParse(bill['cashDiscount']?.toString() ?? '0.0') ?? 0.0;

                      for (var item in items) {
                        double purchaseRate =
                            double.tryParse(item['purchaseRate']?.toString() ?? '0.0') ?? 0.0;
                        double saleRate =
                            double.tryParse(item['saleRate']?.toString() ?? '0.0') ?? 0.0;
                        int quantity =
                            int.tryParse(item['quantity']?.toString() ?? '0') ?? 0;

                        double itemProfit = (saleRate - purchaseRate) * quantity;
                        totalProfit += itemProfit;
                      }

                      // Apply cash discount to total profit
                      double discountAmount = (totalProfit * cashDiscount) / 100;
                          ...
                          child: ExpansionTile(
                            ...
                            children: [
                              ...
                              Padding(
                                padding: const EdgeInsets.all(8.0),
                                child: Text(
                                  'Total Profit after Discount: ${totalProfit.toStringAsFixed(2)}',
                                  style: const TextStyle(
                                      fontSize: 18,
                                      fontWeight: FontWeight.bold),
                                ),
